...
import { collection, query, where, getDocs, doc, getDoc, setDoc, updateDoc, deleteDoc } from "https://www.gstatic.com/firebasejs/12.15.0/firebase-firestore.js";
...
async function loadSubjects() {
    ...
    try {
        const q = query(collection(db, COLLECTIONS.SUBJECTS), where("teacherId", "==", currentUser.id));
        const querySnapshot = await getDocs(q);
        
        tableBody.innerHTML = '';
        if (querySnapshot.empty) {
            tableBody.innerHTML = '<tr><td colspan="3" style="text-align:center;">No subjects found. Add one above!</td></tr>';
            return;
        }
        ...
        document.querySelectorAll('.edit-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                document.getElementById("subjectIdHidden").value = e.target.getAttribute('data-id');
                document.getElementById("subjectNameInput").value = e.target.getAttribute('data-sub');
                document.getElementById("subjectDescInput").value = e.target.getAttribute('data-desc');
                modal.style.display = "flex";
            });
        });
        ...
        document.querySelectorAll('.delete-btn').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                if(!confirm("Are you sure? This deletes the subject permanently.")) return;
                showLoader(true);
                try {
                    await deleteDoc(doc(db, COLLECTIONS.SUBJECTS, e.target.getAttribute('data-id')));
    ...
    try {
        if(subId) {
            ...
            await setDoc(doc(db, COLLECTIONS.SUBJECTS, newId), {
                ...
                teacherPicture: currentUser.picture || 'https://via.placeholder.com/150',
                subject: subName,
                description: subDesc,
                isApproved: true 
            });
