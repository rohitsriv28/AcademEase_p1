...
async function loadPendingTeachers() {
  ...
  try {
    ...
    if (querySnapshot.empty) {
      tableBody.innerHTML =
        '<tr><td colspan="4" style="text-align:center;">No pending approvals.</td></tr>';
      return;
    }
    ...
    document.querySelectorAll(".approve-btn").forEach((btn) => {
      btn.addEventListener("click", (e) =>
        handleApproval(e.target.getAttribute("data-id"), true),
      );
    });
    document.querySelectorAll(".reject-btn").forEach((btn) => {
      btn.addEventListener("click", (e) =>
        handleApproval(e.target.getAttribute("data-id"), false),
      );
    });
  } catch (err) {
    console.error(err);
  ...
  try {
    ...
    if (isApprove) {
      ...
      subjectDocs.forEach((subDoc) => {
        batch.update(doc(db, COLLECTIONS.SUBJECTS, subDoc.id), {
          isApproved: true,
        });
      });
