...
async function loadActiveTeachers() {
    ...
    tableBody.innerHTML = '<tr><td colspan="4" style="text-align:center;">Loading active teachers...</td></tr>';
    
    try {
        const q = query(collection(db, COLLECTIONS.USERS), where("role", "==", ROLES.TEACHER));
        ...
        if (querySnapshot.empty) {
            tableBody.innerHTML = '<tr><td colspan="4" style="text-align:center;">No active teachers found.</td></tr>';
            return;
        }
        ...
        tableBody.innerHTML = '<tr><td colspan="4" style="text-align:center; color:red;">Failed to load.</td></tr>';
    }
}
...
async function openEditModal(teacherId) {
    ...
    try {
        ...
        document.getElementById("teacherUsernameInput").value = teacher.userName || "";
        ...
        document.getElementById("teacherExperienceInput").value = teacher.experience || "";
        document.getElementById("teacherPictureInput").value = "";
    ...
    try {
        ...
        if (pictureFile) {
            try {
                ...
                showToast("Cloudinary Upload failed, using default placeholder: " + e.message, true);
            }
        }
        ...
        if (!teacherId) {
            ...
            if (!password || password.length < 6) {
                showLoader(false);
                return showToast("Password is required and must be at least 6 characters.", true);
            }
            ...
            const oldSubjectsQuery = query(collection(db, COLLECTIONS.SUBJECTS), where("teacherId", "==", teacherId));
...
async function removeTeacher(teacherId) {
    if (!confirm("Are you sure you want to completely remove this teacher and ALL their subjects?")) return;
