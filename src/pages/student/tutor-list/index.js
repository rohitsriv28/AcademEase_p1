  ...
  document.querySelectorAll(".book-btn").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      document.getElementById("bookTeacherId").value =
        e.target.getAttribute("data-teacherid");
      document.getElementById("bookingSubTitle").innerText =
        "Subject: " + e.target.getAttribute("data-subname");
      document.getElementById("bookDate").min = new Date()
        .toISOString()
        .split("T")[0];
...
document.getElementById("searchBtn").addEventListener("click", () => {
  ...
  const filtered = allSubjectsCache.filter(
    (s) =>
  ...
  .addEventListener("click", async () => {
    ...
    try {
      ...
      if (!overlapSnap.empty) {
        showLoader(false);
        return showToast(
          "This tutor is already booked for this specific time.",
          true,
        );
      }
