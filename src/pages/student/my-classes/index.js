...
async function loadBookings() {
  ...
  tableBody.innerHTML =
    '<tr><td colspan="6" style="text-align:center;">Loading booked classes...</td></tr>';
  ...
  try {
    ...
    tableBody.innerHTML =
      '<tr><td colspan="6" style="text-align:center; color:red;">Failed to load bookings.</td></tr>';
  }
}
...
async function renderFilteredBookings() {
  ...
  for (const b of filtered) {
    ...
    const tutorName = tutorDoc.exists()
      ? tutorDoc.data().name
      : "Unknown Tutor";
  ...
  document.querySelectorAll(".reschedule-btn").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      document.getElementById("newDateInput").min = new Date()
        .toISOString()
        .split("T")[0];
...
tabUpcoming.addEventListener("click", (e) =>
  setActiveTab("Upcoming", e.target),
);
tabScheduled.addEventListener("click", (e) =>
  setActiveTab("Scheduled", e.target),
);
tabCompleted.addEventListener("click", (e) =>
  setActiveTab("Completed", e.target),
);
...
document
  ...
  .addEventListener("click", async () => {
    ...
    try {
      ...
      const conflicts = overlapSnap.docs.filter(
        (docSnap) => docSnap.id !== bookingId,
      );

      if (conflicts.length > 0) {
        showLoader(false);
        return showToast(
          "Tutor is already booked for this specific time slot.",
          true,
        );
      }
